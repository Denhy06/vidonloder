export async function scrapeInstagram(url) {
  try {
    ...
    // HELPER: Decode token JWT snapcdn untuk mendeteksi tipe media asli (.mp4 vs .jpg)
      ...
      try {
        if (downloadUrl.includes('token=')) {
          ...
          if (token && token.includes('.')) {
            ...
            // Cek pasti jika ekstensi filenya .mp4
            if (filename.includes('.mp4') || targetUrl.includes('.mp4')) {
              return true;
            }
            // Cek pasti jika ekstensi filenya foto
            if (filename.match(/\.(jpg|jpeg|webp|png)$/) || targetUrl.match(/\.(jpg|jpeg|webp|png)$/)) {
              return false;
            }
          }
        }
