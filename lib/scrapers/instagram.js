export async function scrapeInstagram(url) {
  try {
    ...
    for (const liHtml of liMatches) {
      ...
      // 2. Cari SEMUA tag <a> di dalam item ini
      const aMatches = liHtml.match(/<a\b[^>]*>([\s\S]*?)<\/a>/gi) || [];
      ...
      for (const aHtml of aMatches) {
        ...
        // Cek apakah tombol ini khusus untuk Video
        const isVideoBtn = /\bvideo\b/i.test(aHtml) || 
                           /download video/i.test(aHtml) || 
                           /icon-dlvideo/i.test(liHtml) ||
                           /\.mp4\b/i.test(href);
    ...
    // TAMBAHKAN PRIORITAS: Ambil video dulu jika ada, baru fallback ke gambar
